...
public class ProductsDataService implements DataAccessInterface<ProductModel> {
		...
		try {
			...
			while(srs.next()) {
				products.add(new ProductModel(
						...
						srs.getString(3),//type
						srs.getString(4),//price
						srs.getString(5),//description
						srs.getString(6)//image
						));
			}
		} catch (Exception e) {
	...
	public int create(ProductModel p) {
		...
		String sql = "INSERT INTO products(name, type, price, description, image) VALUES (?, ?, ?, ?, ?)";
		int rows = -1;
		try {
	...
	public int update(ProductModel p) {
		String sql = "UPDATE products SET name = ?, type = ?, price = ?, description = ?, image = ? WHERE products_id = ?";
		int rows = -1;
		try {
			rows = jdbcTemplateObject.update(sql,
					p.getName(),
